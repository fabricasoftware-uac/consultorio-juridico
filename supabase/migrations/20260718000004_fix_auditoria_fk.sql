-- Fix: guardar_entrevista usa id_usuario del caso para auditoria
-- La FK fk_auditoria_usuario apunta a perfiles.id, pero p_usuario_id
-- (auth.uid del estudiante) podria no existir alli en edge cases.
-- Usamos el id_usuario del caso, que siempre esta vinculado al cliente.
...
DECLARE
    v_id_usuario uuid;
BEGIN
    -- Obtener id_usuario del caso para auditoria (mas seguro que p_usuario_id)

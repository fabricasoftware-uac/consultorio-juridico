-- Funcion atomica: guardar entrevista completa (caso + usuario + demandado + contrato + auditoria)
-- Todas las operaciones corren dentro de una sola transaccion PostgreSQL.
-- Si falla cualquier paso, se hace ROLLBACK automatico.
...
BEGIN
    ...
    UPDATE public.usuarios SET
        ...
        estado_civil = NULLIF(p_usuario->>'estado_civil', ''),
        ...
        situacion_laboral = NULLIF(p_usuario->>'situacion_laboral', ''),
        ...
        caracterizacion_lgbtiq = NULLIF(p_usuario->>'caracterizacion_lgbtiq', '')
    ...
    IF p_contrato IS NOT NULL AND NULLIF(p_contrato->>'tipo_contrato', '') IS NOT NULL THEN
        IF EXISTS (SELECT 1 FROM public.contratos_laborales WHERE id_usuario = p_usuario_id) THEN
            UPDATE public.contratos_laborales SET
                tipo_contrato = p_contrato->>'tipo_contrato',

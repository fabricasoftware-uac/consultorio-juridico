-- Fix: casts explícitos de text a enum en guardar_entrevista (restantes)
-- El operador ->> de JSONB devuelve text, pero varias columnas son enums.
-- PostgreSQL no permite cast implícito de text a enum en expresiones dinámicas.
--
-- Campos corregidos en esta migración:
--   - area (casos)           -> public.area_enum
--   - estado_civil (usuarios)  -> public.estado_civil_enum
--   - situacion_laboral (usuarios) -> public.situacion_laboral_enum
--   - caracterizacion_lgbtiq (usuarios) -> public.caracterizacion_lgbtiq_enum
--   - tipo_contrato (contratos_laborales) -> public.tipo_contrato_enum
...
BEGIN
    ...
    IF p_contrato IS NOT NULL AND NULLIF(p_contrato->>'tipo_contrato', '') IS NOT NULL THEN
        IF EXISTS (SELECT 1 FROM public.contratos_laborales WHERE id_usuario = p_usuario_id) THEN

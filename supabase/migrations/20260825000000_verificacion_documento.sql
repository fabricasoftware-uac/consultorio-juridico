-- ============================================================================
-- Verificación del documento del solicitante — Agosto 2026
--
-- El paso 8 de la entrevista pedía "C.C. del Solicitante para Firma" en un
-- input libre. Ese dato:
--   1. era redundante: la cédula ya está en usuarios.cedula, capturada por el
--      profesional de apoyo al crear el caso, donde es obligatoria;
--   2. no se guardaba en ninguna parte (cedulaSolicitante solo vivía en el
--      estado del formulario);
--   3. no se comparaba contra la registrada, así que cualquier número pasaba.
--
-- Se reemplaza por una verificación real: al estudiante se le muestra el
-- documento ya registrado y confirma que coincide con el físico que tiene
-- delante, pudiendo corregirlo si el dato venía mal digitado. Eso sí deja
-- constancia y sirve de control sobre la digitación inicial.
-- ============================================================================

ALTER TABLE public.casos
  ADD COLUMN IF NOT EXISTS documento_verificado     boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS documento_verificado_en  timestamptz,
  ADD COLUMN IF NOT EXISTS documento_verificado_por uuid;

COMMENT ON COLUMN public.casos.documento_verificado IS
  'El estudiante confirmó en la entrevista que el documento físico del solicitante coincide con lo registrado.';

-- ---------------------------------------------------------------------------
-- guardar_entrevista: registra la verificación y persiste una corrección de
-- cédula si el estudiante detectó que el dato registrado estaba mal.
-- ---------------------------------------------------------------------------
...
BEGIN
    ...
    v_verificado := COALESCE((p_caso->>'documento_verificado')::boolean, false);
    ...
    UPDATE public.casos SET
        ...
        documento_verificado = v_verificado,
        -- Solo se sella cuando de verdad se verificó: si no, no se inventa una
        -- fecha de verificación que nadie hizo.
        documento_verificado_en  = CASE WHEN v_verificado THEN now() ELSE NULL END,
        documento_verificado_por = CASE WHEN v_verificado THEN p_usuario_id ELSE NULL END
    WHERE id_caso = p_id_caso;

    -- 2. Actualizar usuarios (cliente, v_id_usuario) con todo el formulario sociodemografico
    UPDATE public.usuarios SET
        -- La cédula solo se toca si el estudiante mandó una corrección; si viene
        -- vacía se conserva la registrada por el profesional de apoyo.
        cedula = COALESCE(NULLIF(p_usuario->>'cedula', ''), cedula),
    ...
    -- propio dejaria al asesor sin aviso. Lo que distingue una correccion de un
    -- envio inicial va en metadata.es_correccion.

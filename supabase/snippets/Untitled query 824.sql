...
-- Requiere que los 4 usuarios ya existan en auth.users / perfiles
-- (creados por `npx tsx scripts/seed.ts`, que dispara handle_new_user).
-- Los perfiles se ubican por su correo (perfiles.correo = auth.users.email).
-- =====================================================================

-- ROLES
INSERT INTO public.perfiles_roles (user_id, role)
VALUES
-- Admin (Luis Hernandez)
...
VALUES
(gen_random_uuid(), 'Juan Pérez', 'MASCULINO', '10023456', '+573004567890', 35, 'María López', 'casado', 3,
 'Calle 10 #20-30', 'juan.perez@example.com', 'propia', 'dependiente', true, 500000, 'ventas ocasionales', true, false),
(gen_random_uuid(), 'Luisa Gómez', 'FEMENINO', '10023457', '+573115678901', 28, 'Carlos Gómez', 'soltero', 2,
 'Cra 15 #25-40', 'luisa.gomez@example.com', 'arriendo', 'independiente', false, NULL, NULL, true, true),
(gen_random_uuid(), 'Pedro Torres', 'MASCULINO', '10023458', '+573224567890', 40, 'Ana Torres', 'unión libre', 1,
 'Av 30 #50-60', 'pedro.torres@example.com', 'propia', 'desempleado', false, NULL, NULL, false, false);

-- CASOS
INSERT INTO public.casos (
   id_usuario, resumen_hechos, observaciones, estado, clasificacion, area, tipo_proceso
)
VALUES
((SELECT id_usuario FROM usuarios WHERE nombre_completo = 'Juan Pérez'),
 ...
 'Pendiente de documentos adicionales.', 'en_proceso', 'en_tramite', 'laboral', 'despido injustificado'),
((SELECT id_usuario FROM usuarios WHERE nombre_completo = 'Luisa Gómez'),
 'Conflicto por cuota alimentaria con su expareja.', 'Esperando citación.',
 'pendiente_aprobacion', 'en_tramite', 'familia', 'cuota alimentaria'),
((SELECT id_usuario FROM usuarios WHERE nombre_completo = 'Pedro Torres'),
 'Demanda civil por incumplimiento de contrato.', 'Caso revisado por asesor.',
 'en_proceso', 'en_tramite', 'civil', 'incumplimiento de contrato');

-- DEMANDADOS
INSERT INTO public.demandados (id_caso, nombre_completo, documento, celular, lugar_residencia, correo)
VALUES
((SELECT c.id_caso FROM casos c JOIN usuarios u ON u.id_usuario = c.id_usuario WHERE u.nombre_completo = 'Juan Pérez'),
 'Empresa XYZ S.A.', '900123456', '+573214567890', 'Bogotá', 'contacto@xyz.com'),
((SELECT c.id_caso FROM casos c JOIN usuarios u ON u.id_usuario = c.id_usuario WHERE u.nombre_completo = 'Luisa Gómez'),
 'José Rodríguez', '1012345678', '+573334567890', 'Medellín', 'jose.rodriguez@example.com'),
((SELECT c.id_caso FROM casos c JOIN usuarios u ON u.id_usuario = c.id_usuario WHERE u.nombre_completo = 'Pedro Torres'),
 'Compañía ABC Ltda.', '800987654', '+573114567890', 'Cali', 'info@abc.com');

-- RELACIONES: ESTUDIANTES_CASOS y ASESORES_CASOS (caso de Juan Pérez)
INSERT INTO public.estudiantes_casos (id_estudiante, id_caso, fecha_asignacion)
VALUES
((SELECT id FROM perfiles WHERE correo = 'estudiante@ejemplo.com'),
 (SELECT c.id_caso FROM casos c JOIN usuarios u ON u.id_usuario = c.id_usuario WHERE u.nombre_completo = 'Juan Pérez'),
 CURRENT_DATE);

INSERT INTO public.asesores_casos (id_asesor, id_caso, fecha_asignacion)
VALUES
((SELECT id FROM perfiles WHERE correo = 'asesor@ejemplo.com'),
 (SELECT c.id_caso FROM casos c JOIN usuarios u ON u.id_usuario = c.id_usuario WHERE u.nombre_completo = 'Juan Pérez'),
 CURRENT_DATE);
